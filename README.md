...
.py script with the default capability to download and locally store all files from LBRY listed in guncadindex.com "release" API, along with the index's metadata for these files. Creates a folder structure and a synchronized excel database to enable user friendly methods to locate files saved to a user-specified filed path 
...
GunCAD Index does not host, control, or distribute any linked content. It is purely an index and search engine. You access any sites linked here at your own risk and subject to their terms and policies
